...
class CephRBDMirrorControlledFailoverTest(CephRBDMirrorBase):
    ...
    def execute_failover_juju_actions(self,
        ...
        logging.info(result)
        zaza_utils.assertActionRanOK(result)

        # Validate that the demoted pools count matches the total primary site
        # pools count.
        n_pools_demoted = len(result.results.get('output').split('\n'))
        ...
        # site pools count.
        n_pools_promoted = len(result.results.get('output').split('\n'))
